...
ecosystems out of the box. Free and open source (Apache-2.0), running on
...
## Prun vs. other cleanup tools

There are good tools in this space, and for some jobs they're the better
pick. Roughly where Prun sits:

| | **Prun** | **[npkill](https://github.com/voidcosmos/npkill)** | **[kondo](https://github.com/tbillington/kondo)** | **ncdu / WizTree** | **CleanMyMac & co.** |
|---|---|---|---|---|---|
| **Scope** | ~26 ecosystems | `node_modules` | 20+ project types | any large dir | system-wide |
| **Knows what's safe to delete** | yes | yes (Node only) | yes | no — size only | heuristic |
| **Interface** | desktop GUI **and** CLI | CLI | CLI/TUI (+ separate GUI) | TUI | GUI |
| **Trash by default** | yes | no | no | n/a | varies |
| **Skips git-tracked dirs** | yes | no | no | n/a | no |
| **`.prunignore` exclusions** | yes | no | no | n/a | no |
| **Editable detection rules** | yes, in-app | no | no | n/a | no |
| **Platforms** | Win / macOS / Linux | Win / macOS / Linux | Win / macOS / Linux | varies | macOS |
| **Price** | free, Apache-2.0 | free, MIT | free, MIT | free | commercial |
| **Prebuilt binary** | not yet — build from source | `npx npkill` | yes | yes | yes |

**When *not* to reach for Prun:**

- You only ever touch Node projects and want zero setup — `npx npkill` is one
  command and needs nothing installed.
- You want a binary today. Prun has no published release yet; you build it
  from source (Node + Rust toolchain). kondo installs with `cargo install kondo`.
- You're hunting general disk hogs — photos, VMs, downloads. Prun deliberately
  only ever proposes regeneratable build artifacts; use a disk analyzer.
- You're on a terminal-only box and want a TUI. Prun's CLI is non-interactive
  by design (scan / dry-run / clean), not a browsable interface.

**When Prun is the better fit:** many projects across several languages, and
you want to *see* what you're removing grouped by project before it goes —
with Trash as the default, git-tracked directories left alone, and detection
rules you can change without editing anyone's source.
...
### How do I delete `node_modules` from all my projects at once?

Point Prun at the folder that contains your projects. It walks the whole
tree, finds every `node_modules` — along with the other artifacts each
project left behind — and lists them grouped by project with sizes. Select
what you want and clean it in one pass. From the CLI:
`prun scan ~/Projects`, then `prun clean --scan ~/Projects --yes`.

### Is it safe to delete `node_modules`, `target`, and `.venv`?

Yes — they're regenerated by your next install or build, which is why Prun
only ever proposes artifacts of that kind. Three further guards: cleaning
moves to the system Trash by default (recoverable), git-*tracked* directories
are skipped (a tracked build dir may be intentional), and a `.prunignore`
excludes paths permanently. A permanent delete is opt-in and needs a second,
confirming click.

### Which languages and build artifacts does Prun detect?

~26 ecosystems: Rust (`target`), Node.js (`node_modules`, `.next`, `dist`),
Python (`.venv`, `__pycache__`), JVM/Gradle (`build`, `.gradle`), Go, .NET
(`bin`, `obj`), PHP (`vendor`), Ruby, C/C++ and CMake, Swift/Xcode
(`DerivedData`), Dart, Zig, Haskell, Elixir/BEAM, Nim, Crystal, Bazel, Nix,
LaTeX — plus per-user package caches (Cargo registry, npm, Gradle) and
cross-cutting junk like OS files and editor swap files. The full list is
[`src-tauri/prun-rules.toml`](src-tauri/prun-rules.toml).

### Can Prun run unattended in a script or cron job?

Yes. A scan-driven clean defaults to a dry run unless you pass `--yes`, so a
cron job can start by printing what it *would* do. `--json` gives
machine-readable output; `--delete` removes permanently instead of trashing,
which is the usual choice for reclaiming CI cache space. See
[Command-line use](#command-line-use).

### How do I stop Prun from deleting a folder I need?

Three ways, in increasing permanence: leave it unselected (nothing is cleaned
unless you select it); add a `.prunignore` at the scan root using gitignore
syntax; or disable/edit the matching rule in the **Rules** tab — saved as an
override on top of the built-in ruleset, so built-in updates still reach you.

### Does Prun delete anything without asking?

No. A scan only reads. The GUI cleans exactly what you've ticked, and a
permanent delete arms the button for a second, confirming click. The CLI
cleans explicit paths you named, and a `--scan` clean is a dry run until you
add `--yes`.
